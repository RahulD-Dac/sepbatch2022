 ...
 static  ListNode reverse( ListNode head )
{
			...
			while(curr!= null)
			{
				temp = curr.next;	 // temp goes to next of curr
				curr.next =previous;  // link break and previous la connect 
				previous = curr;      // curr chi value previous la save k le 		
				curr = temp ; 			// curr la travesr to temp which is next node
			}		
	
	return previous ;  /// end la head gela ani null to start 
 }
  ...
  public static void main (String [] args)
   {
	   SinglyLinkedList sll= new SinglyLinkedList();
		...
		//ListNode second = new ListNode(22);       // creating refrance for Node and memory allocation
		//ListNode thrid = new ListNode(33);  			 
				
				
		//sll.head.next = second;   // linkinfg node
		//second.next = thrid;
		// sll.display();      			 
	  ...
	  // sll.deleteNode(17);
	   //sll.deleteNodePos(2);
		//sll.seach(head, 00);
		  ...
		  sll.display();
		  head=reverse(head);         // head pass  save the result in head its imp
		  sll.display();
